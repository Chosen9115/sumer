...
//! leaves the LAST SUCCESSFUL read's row on top -- still stamped `live` --
//! unless something writes the fact that this refresh did not read it. The
//! refresh error is reported, but a reported error and a `live` figure are
//! two different things arriving in two different places, and only one of
//! them is the number the user acts on.
...
/// **A refresh that fails BEFORE `balances.read` must still mark what it
/// did not read.**
///
/// `status.read` runs before the balances do, and its failure returns from
/// the refresh early. The marker that downgrades an unread figure was
/// written only inside the `balances.read` arm, so every path that returned
/// before it -- a spawn failure, a `resources.list` failure, this one --
/// left the last successful read's row on top. One good read followed by an
/// unbounded run of failures printed a `live` figure from a read that
/// happened days ago, for ever.
#[tokio::test]
async fn a_failure_before_balances_read_does_not_leave_the_figure_live() {
    ...
    assert!(
        after.outcome.starts_with("unread:"),
        "a read that did not happen must not go on reading `live`; got {:?}",
        after.outcome
    );
    assert!(
        after.amount.is_none(),
        "and the marker invents no figure of its own"
    );
    assert_eq!(
        format!("{:?}", after.staleness),
        "Unavailable",
        "the freshness the renderer reads is downgraded, not the error log alone"
    );
}
