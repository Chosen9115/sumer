...
`next: null`) so the adapter writes its `seen.json`, and waits for it to
exit. The LAST index is then spawned with our own stdin and stdout, so the
conformance crawl talks to it directly.

Why this exists: the tombstone and stale cases are two adapter lifetimes
over one `--state-dir`, and the conformance runner spawns exactly one argv
per execution. This makes "the phase-2 adapter, with a state file phase 1
actually wrote" a single command line.

Why the state directory is FRESH on every launch: the suite runs each case
twice and compares the two executions (A9). Phase 2 rewrites `seen.json` --
a tombstoned transaction is dropped from the baseline -- so a shared
directory would make the second execution differ from the first. Re-priming
from an empty directory makes both executions the same two-lifetime
scenario, which is what the fixture describes.
